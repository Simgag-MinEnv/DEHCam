//./build-local.sh main
//particle flash --usb target/1.5.0-rc.2/boron/BoronCamDEH.bin
//particle flash --usb target/1.4.4/boron/BoronCamDEH.bin
//particle flash 44515 target/1.5.0-rc.2/boron/BoronCamDEH.bin
...
#define VERSION_SLUG "V1.3" //2020/03/09
      ...
      while(bytesRead <= len) {
        ...
        if(result == 0) {
          ...
          while(result == 0 ) {
            ...
            Serial.println("write error " + String(writeerror));           
            Particle.process();
            result = ftp.data.write(buf,100);
            ftp.data.flush_buffer();
            ClientTries -= 1 ;
